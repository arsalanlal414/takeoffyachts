...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-black text-white z-50 flex flex-col justify-between lg:hidden w-[250px]"
            ...
            <nav className="px-6 flex flex-col gap-2">
              {menuItems.map((item) => (
                <NavLink
                  ...
                  key={item.name}
                  className={({ isActive }) =>
                    `block py-2 px-4 w-full text-base sm:text-lg rounded-lg flex gap-2 items-center ${
                      isActive ? "bg-[#BEA355]" : "hover:bg-gray-700"
                    }`
                  }
